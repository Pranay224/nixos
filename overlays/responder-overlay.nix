final: prev: {
  responder-patched = prev.responder.overrideAttrs (oldAttrs: {
    ...
    installPhase = ''
      ...
      makeWrapper ${prev.python3.interpreter} $out/bin/responder \
        ...
        --run "${prev.coreutils}/bin/mkdir -p /var/lib/responder"
    ...
    postInstall = ''
      wrapProgram $out/bin/responder \
        --prefix PATH : "${final.openssl}/bin:${final.coreutils}/bin" \
        --run "${final.coreutils}/bin/mkdir -p /var/lib/responder/certs && \
               ${final.openssl}/bin/openssl genrsa -out /var/lib/responder/certs/responder.key 2048 && \
               ${final.openssl}/bin/openssl req -new -x509 -days 3650 \
                   -key /var/lib/responder/certs/responder.key \
                   -out /var/lib/responder/certs/responder.crt -subj '/'" \
        --run "${prev.coreutils}/bin/mkdir -p /etc/responder && \
               if [ ! -f /etc/responder/Responder.conf ]; then \
                 cp $out/share/Responder/Responder.conf /etc/responder/Responder.conf && \
                 chmod +w /etc/responder/Responder.conf; \
               fi"
    '';
